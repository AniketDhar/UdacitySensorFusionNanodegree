...
range = 110;        % initial range of target
velocity = -30;     % initial velocity of target 
...
Tchirp = 5.5 * 2 * max_range / c;   % Chirp time (5.5 for margin)
...
sig_fft1 = sig_fft1(1:Nr/2, :);  % Single-sided
...
Tr = 10; Tc = 8;   % Training Cells (Range, Doppler)

%Select the number of Guard Cells in both dimensions around the Cell under 
%test (CUT) for accurate estimation
Gr = 4; Gc = 4;    % Guard Cells

% offset the threshold by SNR value in dB
offset = 6;        % SNR threshold in dB
...
CFAR = zeros(size(RDM));  % Initialize CFAR output map

for i = Tr+Gr+1 : (Nr/2 - (Tr+Gr))
    for j = Tc+Gc+1 : (Nd - (Tc+Gc))
        noise_level = 0;

        % loop over the training + guard window
        ...
        % Calculate threshold
        num_train_cells = ((2*(Tr+Gr)+1)*(2*(Tc+Gc)+1)) - ((2*Gr+1)*(2*Gc+1));
        threshold = pow2db(noise_level / num_train_cells);
