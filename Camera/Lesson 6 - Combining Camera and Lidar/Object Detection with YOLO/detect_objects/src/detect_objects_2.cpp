...
void detectObjects2()
{
    ...
    float nmsThreshold = 0.35;  // Non-maximum suppression threshold
    vector<int> indices;
    ...
    for (auto it = bBoxes.begin(); it != bBoxes.end(); ++it)
    {
        ...
        int baseLine;
        cv::Size labelSize = getTextSize(label, cv::FONT_ITALIC, 0.25, 1, &baseLine);
        ...
        cv::putText(visImg, label, cv::Point(left, top), cv::FONT_ITALIC, 0.25, cv::Scalar(0, 0, 0), 1);
    }
